...
;   M98 P"ProbeZ-.g"       ; "Set Z" makes the probed surface Z0
;   M98 P"ProbeZ-.g" S10   ; "Set Z" makes the probed surface Z10
;
; A dialog shows the measured height with two options:
;   1. Set Z  - set the probed surface to the target value in the active WCS
;   2. Close  - just report, change nothing
...
; Ask what to do with the measurement
M291 P{"Measured Z = " ^ var.measuredZ ^ " mm (current WCS)."} R"Probe Z" S4 K{"Set Z" ^ var.targetZ, "Close"}

if input == 0
    ; Current position is var.retract above the surface; make the surface read var.targetZ
    G10 L20 Z{var.targetZ + var.retract}
    echo "Probed surface set to Z", var.targetZ, "in the current work coordinate system."
else
