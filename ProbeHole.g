...
; touches X+, X-, Y+, Y- and computes the hole center and diameter, then offers to
; set the center as X0 Y0 (or X0 only / Y0 only) in the CURRENTLY ACTIVE work
; coordinate system.
;
...
M291 P{"Hole center X = " ^ var.centerX ^ ", Y = " ^ var.centerY ^ " mm (current WCS). Diameter = " ^ var.dia ^ " mm."} R"Probe Hole" S4 K{"SetX0Y0", "Set X0", "Set Y0", "Close"}

; The probe is sitting at the computed center, so G10 L20 applies directly
if input == 0
    G10 L20 X0 Y0
    echo "Hole center set as X0 Y0 in the current work coordinate system."
elif input == 1
    G10 L20 X0
    echo "Hole center set as X0 in the current work coordinate system."
elif input == 2
    G10 L20 Y0
    echo "Hole center set as Y0 in the current work coordinate system."
